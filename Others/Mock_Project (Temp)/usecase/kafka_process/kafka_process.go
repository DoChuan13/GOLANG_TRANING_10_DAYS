...
var totalCount, reCount, countProcess = 0, 0, 0
...
func (s Server) StartKafkaProcess(csCh chan model.ConsumerObject, done chan bool, rows []string) error {
	...
	for i := 0; i < len(rows); i++ {
		...
		if !isExists {
			...
			s.wg.Add(1)
			go s.consumerProcess(csCh, topicName, 0)
		}
	...
	isComplete := false
	for !isComplete {
		if countProcess == totalCount {
			fmt.Println("First End")
			for topic, _ := range s.config.Topics {
				EOF := model.EOF
				s.wg.Add(1)
				go s.producerProcess(topic, EOF, 0)
			}
			isComplete = true
		}
	}
